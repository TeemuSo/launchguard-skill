...
> **⚠️ SAFETY-CRITICAL — a keyless cross-tenant run reads `fixed` but proves NOTHING.** The second-identity mint is **`requiresPro`** (and also fails on the non-Supabase-Auth / `sb_publishable_` cases above). When the engine **can't mint** that identity, it does NOT error loudly — the request just goes out **keyless** → the app 401s → the run reads **`fixed`** with **`credentialProvenance: "none"`** (or a body like `"No API key found"`). **That `fixed` is NOT a pass — it's a credential ceiling (an unmet precondition), so cross-tenant isolation was never actually tested.** **Rule: before trusting ANY cross-tenant / authed `fixed`, CHECK `credentialProvenance` (and the response body).** If it's `none` / keyless, treat the verdict as **inconclusive / untested**, never "isolation verified" — and **never report cross-tenant isolation as PASS when the second identity wasn't actually provisioned.** Cross-reference `reference/methodology.md` Step 7 (#6, the credential ceiling) and `reference/invariants.md` B1.
...
Validation note: because the engine signs up its OWN throwaway account for this template, you cannot reproduce that identity with a plain curl. For `crossTenant` / `anonKey` chains the `/run` verdict IS the validation — **but only once you've confirmed the identity was actually minted.** A `fixed` here is trustworthy ONLY if `credentialProvenance` shows a real provisioned identity; a `none` / keyless `fixed` is the credential ceiling above (the signup failed and the app just 401'd an anonymous request), so treat it as untested, not "isolation verified".
...
Then **close the loop**: apply the fix to the project, and re-run `POST /api/v1/chains/<id>/run` to confirm the verdict flips from `vulnerable` to `fixed`. That re-run is the external proof the fix actually landed, the chain equivalent of the scan loop in `SKILL.md` Step 6 (code fix, then external proof). If it still reads `vulnerable`, the fix did not hold; iterate.
