...
> **Want to prove a flow WORKS, not that it's exploitable?** That is a *functional* test (a Playwright **script** chain with `intent:"functional"`, where PASS = working/green and FAIL = broken/red), not the HTTP request-plus-matcher chain documented below. Read `reference/functional-methodology.md` for the two-gate authoring pattern and the functional verdict mapping. The HTTP-chain mechanics in this file are for security exploits; functional regression chains are authored as scripts and are the right tool for "watch that my signup / checkout / dashboard still works on every deploy." **Plan note: an HTTP request-plus-matcher chain runs FREE and unbounded (including HTTP cross-tenant); a script (real-browser) chain is PRO to run — a non-Pro caller gets `402 browser_testing_requires_pro`, and its watched deploy re-runs draw Pro browser-minutes. So when authoring for a FREE or unknown-plan account, DEFAULT TO an HTTP-artifact chain; author a script/browser chain only when the account is confirmed Pro (or hand the browser proof over as the local LaunchProof harness, which runs on the user's own machine for free).**
...
- **Proof vs Guard** — the `watched` boolean (one-shot Proof vs deploy-replay Guard); `lastResult` values
- **The format** — one request + a matcher as JSON; the `interpretation` block (what the founder reads); RED vs GREEN framing
- **Authoring a chain** — Step 2 list + pre-flight quota, Step 3 anonymous-read template, Step 3b cross-tenant template, the witnessed-GREEN gate-guard recipe, footguns, Step 4 submit/run, Step 5 fix prompt
- **Cleanup / triage pass** — archive decision rules; custom chains vs. the default scanner
- **The full lifecycle** — list / inspect / create / PATCH / run / archive-restore; mutation runs; captured-session tests; disposition
...
> **Pre-flight quota check (free tier), before expensive authoring.** Read your saved-chain budget with `GET /api/v1/me` (contract: `reference/chains-reference.md` §7). `chains.remaining === 0` on a free key means the next ingest is refused `402 pro_required`, `reason: "free_chain_limit"` (free accounts keep 2 active saved chains). At the cap, archive a chain to free a slot, save only the highest-impact test, or surface the Pro upgrade — don't do the authoring work only to be refused at ingest. A Pro key returns `remaining: null` (unlimited).
...
>
> **The Pro line is browser/custody, not the mint.** The HTTP cross-tenant chain here (the inline anon-signup) is free and unbounded. The Pro signals live on the **browser / captured-session** path: any non-Pro real-browser (script) chain run → `402 browser_testing_requires_pro` (real-browser testing is Pro; there is no free browser proof), and running a chain that carries a STORED credential → `402 stored_credentials_require_pro`. Branch on whichever live server signal you actually get (`requiresPro` / `402` / `locked` / `inconclusive`), never a hardcoded tier. **One more free-tier bound: free accounts keep up to 2 active saved tests (custom chains).** Authoring and running are free; *storing* is the gate (a saved chain is memory). A non-Pro account that already holds 2 active chains is refused a 3rd at ingest: `402 pro_required`, `reason: "free_chain_limit"`. Archiving a chain frees a slot; Pro is unlimited saved chains.
...
> **A script-chain `/run` launches a REAL remote browser session that costs real money and takes about two minutes, so never fire it casually or in a loop.** The run is persisted the moment it finishes, so treat the `/run` response as precious and save it to a file (it carries the `runId`). To re-read a past verdict, call `GET /api/v1/chains/<chainId>/runs/<runId>` with the `runId` you already hold instead of re-POSTing `/run` and paying again; lost the `runId`? `GET /api/v1/chains/<chainId>/runs` lists recent runs so you can recover it.
...
**Captured-session (authenticated) tests** — for "logged in but under-privileged" bugs the engine can't mint an identity for (a Pro-only route reachable by a free user, an admin function reachable by a member, an authed IDOR on a Clerk/Auth0/Firebase app): upload a Playwright `storageState` with `POST /api/v1/chains/credentials` and reference the returned `credentialId` from a **script** chain that runs authenticated as that real identity. The script-chain body is FLAT (top-level `artifact:"script"`, the Playwright spec as a top-level `script` STRING that declares its intent via native Playwright metadata in the test-details object — a `@security` / `@functional` tag, plus a `{ type: 'secure-when', description: 'pass' }` annotation for a security test) — get it exactly right per `reference/chains-reference.md` §9 (credentials) + §12 (script-chain ingest), or the HTTP runner silently wins, the script never runs, and the run routes to a false `inconclusive`. **Plan gate: a captured-session chain is a real-browser (script) run carrying a stored credential, so it is PRO on both counts — a non-Pro caller gets `402 browser_testing_requires_pro` (real-browser run) and `402 stored_credentials_require_pro` (stored credential). Author and hand over a captured-session chain only when the account is confirmed Pro; for a FREE or unknown-plan account, DEFAULT TO an HTTP-artifact chain (the anonymous-key HTTP cross-tenant read is free and unbounded) and surface the captured-session test as the Pro upgrade path.**
