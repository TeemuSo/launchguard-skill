...
- **`INIT.md`** — set up continuous black-box e2e in the USER'S OWN repo and CI, end to end, for them. Triggers: "set up LaunchGuard on this repo", "wire LaunchGuard into my CI", "watch this app on every deploy from my repo", "init LaunchGuard". You do the work (copy the kit, wire the preview env, author the first watched tests, prove fail-on-base, post the proof), never hand the user a checklist. Reads `reference/environment-and-ci.md`.
...
- **`reference/functional-methodology.md`** — the discipline for functional tests (does a critical journey still work): the two-gate pattern, determinism, and PASS-means-working verdict mapping.
- **`reference/environment-and-ci.md`** — how to RUN black-box e2e in the user's own pipeline: target the per-PR deployment (never a branch alias), give it a working backend by reusing the app's env-driven non-prod config (no app code changes), the Vercel bypass + `deployment_status` targeting, why shared-backend webhooks resolve without reaching the preview, and the two-tier CI that fixes a slow suite. **Read it before wiring a repo's continuous testing.**
