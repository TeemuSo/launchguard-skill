...
> **The script chain itself is ingested through `chains-reference.md` §12 — the script-chain ingest contract — read it before authoring one.** The body is FLAT (no `spec` object): top-level `artifact:"script"`, the Playwright spec as a top-level `script` STRING (stored/read back at `spec.script.source`), the `// @lg-intent:` / `// @lg-secure-when:` header tags declared in the source (**colons required**), and the captured session referenced by top-level `credentialId`. **Critical footgun:** if you send an HTTP `spec.steps` / `spec.assertion` instead of (or alongside, without `artifact:"script"`) the script, the HTTP runner silently wins and your Playwright script is never run — it ingests `201` and routes the run to a false `inconclusive`. This is the single trap that makes authenticated/functional chains feel un-authorable; §12 is the cure.
