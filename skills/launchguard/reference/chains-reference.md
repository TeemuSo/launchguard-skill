...
- All `/api/v1/*` routes (the chain routes and `/api/v1/connect`) are served at `https://api.launchguard.dev` (the public edge).
...
These three read-only/config routes are the **bridge loop**: connect, then READ `/context` so you start expert (the engine already scanned the app), then ACT on its `recommendations[]`, then author / toggle only the gaps. SKILL.md gives the loop; this section is the full contract. All require `Authorization: Bearer $LAUNCHGUARD_API_KEY` and use the same base URL as the chain routes (`https://api.launchguard.dev`). `/context` is `lg_`-key-callable (dogfood-confirmed); it is no longer human-session-only.
